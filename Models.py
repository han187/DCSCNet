...
class DCSCNet(nn.Module):
    def __init__(self,k,d,in_channels=None,class_num=None):
        ...
        self.filters1 = nn.ParameterList(
            [nn.Parameter(torch.randn(self.k,self.dcsc_ini_channels+self.k*i,3), requires_grad=True) for i in range(self.d)])

        self.b0 = nn.Parameter(torch.zeros(1,16,1), requires_grad=True)
        self.b1 = nn.ParameterList(
            [nn.Parameter(torch.zeros(1,self.dcsc_ini_channels+self.k+self.k*i,1),requires_grad=True) for i in range(self.d)])

        self.bn0 = nn.BatchNorm1d(16, affine=True).cuda()
        self.bn1 = [nn.BatchNorm1d(self.dcsc_ini_channels+(i+1)*self.k, affine=True).cuda() for i in range(self.d)]
